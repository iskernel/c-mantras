...
struct cm_operation_status* cm_operation_status_create(
        enum cm_operation_result_code status, const char* message)
{
    ...
    cm_error_log_critical_if((op_status == NULL), __func__,
                             ": not enough memory");
...
struct cm_operation_stack* cm_operation_stack_create(size_t stack_size)
{
    ...
    cm_error_log_critical_if((op_stack == NULL), __func__,
                             ": not enough memory");
    op_stack->length = stack_size;
    op_stack->index = 0U;
    op_stack->data = malloc(sizeof(struct cm_operation_status) * stack_size);
    cm_error_log_critical_if((op_stack->data == NULL), __func__,
                             ": not enough memory");
...
void cm_operation_stack_set_last(struct cm_operation_stack* stack,
                                 struct cm_operation_status* operation)
{
    cm_error_log_critical_if((stack == NULL), __func__, ": stack is null");
    cm_error_log_critical_if((operation == NULL), __func__,
                             ": operation is null");
...
struct cm_operation_status* cm_operation_stack_get_last(
        struct cm_operation_stack* stack)
{
