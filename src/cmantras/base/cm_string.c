...
static struct cm_string* cm_string_create_from_char_array(char* content)
{
	struct cm_string* string = NULL;
    cm_size_u length = 0;
...
static struct cm_string* cm_string_create_from_other_string(struct cm_string* string)
{
	struct cm_string* cmstring = NULL;

    cmstring = cm_string_create(string->data->content,
    							string->data->length);
...
static struct cm_string* cm_string_create_from_char(char character)
{
	struct cm_string* string = NULL;
...
static struct cm_string* cm_string_create_from_integer(int64_t number, uint8_t radix)
{
    ...
    converted_content = malloc(sizeof(char)
...
static struct cm_string* cm_string_create_from_real(double number)
{
...
static cm_cmp_size_i cm_string_compare(struct cm_string* string1, struct cm_string* string2)
{
	cm_cmp_size_i result = strcmp(string1->data->content, string2->data->content);
    return result;
}


static cm_cmp_size_i cm_string_compare_insensitive(struct cm_string* string1, struct cm_string* string2)
{
    cm_cmp_size_i result = 0;
    char* content1 = NULL;
    char* content2 = NULL;

    memcpy(content1, string1->data->content, string1->data->length * sizeof(char));
    memcpy(content2, string2->data->content, string2->data->length * sizeof(char));
...
static double cm_string_compare_ordinal(struct cm_string* string1, struct cm_string* string2)
{
    double nr1 = 0.0;
    double nr2 = 0.0;
    double result = 0.0;
...
static bool cm_string_are_equal(struct cm_string* string1, struct cm_string* string2)
{
    bool result = false;
...
static bool cm_string_contains(struct cm_string* source, struct cm_string* substring)
{
...
static bool cm_string_contains_insensitive(struct cm_string* source, struct cm_string* substring)
{
	...
	memcpy(source_content, source->data->content, source->data->length * sizeof(char));
	memcpy(substring_content, substring->data->content, substring->data->length * sizeof(char));
...
static bool cm_string_ends_with(struct cm_string* source, struct cm_string* substring)
{
    bool result = (substring->data->length <= source->data->length)
                  && (strcmp(source->data->content + source->data->length - substring->data->length,
                             substring->data->content) == 0);
    return result;
}


static bool cm_string_ends_with_insensitive(struct cm_string* source, struct cm_string* substring)
{
	char* source_content = NULL;
	char* substring_content = NULL;
    ...
    bool result =   (substring->data->length <= source->data->length)
                    && (strcmp(source_content + source->data->length - substring->data->length,
						substring_content) == 0);
    return result;
}


static bool cm_string_starts_with(struct cm_string* source, struct cm_string* substring)
{
	bool result = (substring->data->length <= source->data->length)
	               && (strncmp(source->data->content, substring->data->content, substring->data->length) == 0);
	return result;
}

static bool cm_string_starts_with_insensitive(struct cm_string* source, struct cm_string* substring)
{
	char* source_content = NULL;
	char* substring_content = NULL;
	...
	bool result = (substring->data->length <= source->data->length)
				   && (strncmp(source_content, substring_content, substring->data->length) == 0);
	return result;
}
...
static struct cm_object* cm_string_to_cm_object(struct cm_string* string)
{
	...
	interface = cm_object_interface_create(&(*cm_string_destroy),
										   &cm_string_are_equal,
										   &cm_string_compare,
										   &cm_string_create_from_other_string);
...
void module_cm_string_initialize(void)
{
	...
	DEFAULT_CM_STRING_INTERFACE->from_string = &cm_string_create_from_other_string;
