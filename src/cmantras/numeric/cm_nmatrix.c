...
struct cm_nmatrix* cm_nmatrix_copy(const struct cm_nmatrix *source)
{
	...
	result = cm_nmatrix_create(result, source->data->rows, source->data->columns);
	if (result != NULL)
	{
...
struct cm_nmatrix* cm_nmatrix_sum(const struct cm_nmatrix* mat1, const struct cm_nmatrix* mat2)
{
	integer i = 0, j = 0;
	struct cm_nmatrix *result = NULL;
	if ((mat1->data->rows == mat2->data->rows) && (mat1->data->columns == mat2->data->columns))
	{
		result = cm_nmatrix_create(result, mat1->data->rows, mat1->data->columns);
		for (i = 0; i < mat1->data->rows; i++)
		{
			for (j = 0; j < mat1->data->columns; j++)
			{
				result->data->content[i][j] = mat1->data->content[i][j] + mat2->data->content[i][j];
			}
		}
	}
	return result;
...
struct cm_nmatrix* cm_nmatrix_multiply_with_scalar(const struct cm_nmatrix* mat, real value)
{
	integer index_i = 0;
	integer index_j = 0;
...
struct cm_nmatrix* cm_nmatrix_product(const struct cm_nmatrix* mat1, const struct cm_nmatrix* mat2)
{
	integer i = 0, j = 0, k = 0, p = 0;
	struct cm_nmatrix *result = NULL;
	if (mat1->data->rows == mat2->data->columns)
	{
		result = cm_nmatrix_create(result, mat1->data->rows, mat2->data->columns);
		for (i = 0; i < mat1->data->rows; i++)
		{
...
struct cm_narray* cm_nmatrix_get_primary_diagonal_as_array(const struct cm_nmatrix *mat)
{
	integer i = 0;
...
struct cm_narray* cm_nmatrix_get_secondary_diagonal_as_array(const struct cm_nmatrix* mat)
{
	integer i = 0;
	struct cm_narray *array = NULL;
	if (mat->data->rows == mat->data->columns)
	{
		...
		for (i = 0; i < mat->data->rows; i++)
		{
			array->data->content[i] = mat->data->content[i][mat->data->rows - 1 - i];
		}
	}
	return array;
}

struct cm_nmatrix* cm_nmatrix_minor(const struct cm_nmatrix *mat, integer row, integer column,
							 	 	integer order)
{
	integer i = 0, j = 0;
	...
	if ((mat->data->rows == mat->data->columns) && ((row - 1) < order)
			&& ((column - 1) < order) && (order >= 1) && (order < mat->data->rows))
	{
		if (order == 1)
		{
			...
			for (i = 0; i <= order; i++)
			{
				if (i != row)
				{
					...
					for (j = 0; j <= order; j++)
					{
						if (j != column)
						{
							minor->data->content[rowCount][colCount] = mat->data->content[i][j];
							colCount++;
						}
					}
					rowCount++;
...
real cm_nmatrix_determinant(const struct cm_nmatrix *mat, integer order)
{
	...
	real det = 0.0f;
	struct cm_nmatrix *minor = NULL;
	if ((mat->data->rows == mat->data->columns) && (order >= 1) && (order <= mat->data->rows))
	{
		if (order == 1)
		{
			...
			for (k = 0; k < order; k++)
			{
				...
				for (i = 1; i < order; i++)
				{
					...
					for (j = 0; j < order; j++)
					{
						if (j != k)
						{
							minor->data->content[i - 1][l] = mat->data->content[i][j];
							l++;
						}
					}
				}
...
struct cm_nmatrix* cm_nmatrix_transpose(const struct cm_nmatrix* mat)
{
	...
	transpose = cm_nmatrix_create(transpose, mat->data->columns, mat->data->rows);
