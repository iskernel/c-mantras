...
bool cm_arrayset_add(struct cm_arrayset* set, const void* element)
{
    ...
    cm_error_log_critical_if((set == NULL), __func__, "set is null");
    cm_error_log_critical_if((element == NULL), __func__, "element is null");
    cm_error_log_critical_if((set->size > set->capacity), __func__,
                             "Capacity reached");
                                        ...
                                        const struct cm_arrayset* set2)
{
    ...
    result = cm_arrayset_create(set1->size + set2->size, set1->compare);
        ...
        const struct cm_arrayset* secondary_set)
{
    ...
    cm_error_log_critical_if((secondary_set == NULL), __func__,
                             "secondary set is null");
    ...
    if (result != NULL)
    {
        for (i = 0; i < main_set->size; i++)
        {
            ...
            {
                if (main_set->compare(main_set->content[i],
                                      secondary_set->content[j]) == 0)
                {
                    was_found = true;
                    break;
                }
