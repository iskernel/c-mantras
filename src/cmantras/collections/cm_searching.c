...
cm_index cm_search_array_linear(
        void* key, void** array, cm_size array_size,
...
cm_index cm_search_array_sentinel_linear(
        void* key, void** array, cm_size array_size,
        cm_cmp_size (*compare)(const void*, const void*))
{
    cm_index index = 0;
...
cm_index cm_search_array_binary(
        void* key, void** array, cm_size array_size,
        cm_cmp_size (*compare)(const void*, const void*))
{
    cm_index left = 0;
...
cm_index cm_search_array_improved_binary(
        void* key, void** array, cm_size array_size,
        cm_cmp_size (*compare)(const void*, const void*))
{
    cm_index left = 0;
    ...
    if ((compare(key, array[left]) >= 0) && (compare(key, array[right]) <= 0))
    {
        while (left < right)
        {
            (compare(key, array[middle]) > 0) ? (left = middle + 1) : (right =
                                                        middle);
            middle = (left + right) / 2;
        }
...
cm_index cm_search_array_interpolation(
        void* key, void** array, cm_size array_size,
        cm_cmp_size (*compare)(const void*, const void*))
{
    cm_index left = 0;
    cm_index right = array_size - 1;
    cm_index result = -1;
    cm_index middle = left
            + compare(key, array[left]) * (right - left)
    ...
    if ((compare(key, array[left]) >= 0) && (compare(key, array[right]) <= 0))
    {
        while ((compare(array[middle], key) != 0) && (left < right)
