...
struct cm_array* cm_array_create(cm_size capacity)
{
    ...
    cm_error_log_critical_if((array->elements == NULL), __func__,
                             ": not enough memory");
...
bool (*match)(const void* element_1, const void* element_2))
{
    ...
    cm_error_log_critical_if((array == NULL), __func__, ": array is NULL");
    cm_error_log_critical_if((element == NULL), __func__, ": element is NULL");
    cm_error_log_critical_if((match == NULL), __func__,
                             ": search criteria is NULL");

    for (index = 0; index < array->capacity; index++)
    {
        if ((array->elements[index] != NULL)
                && (match(element, array->elements[index]) == true))
        {
            it_contains = true;
            break;
        }
...
void cm_array_sort(
        struct cm_array* array,
        cm_cmp_size (*compare)(const void* element_1, const void* element_2))
{
    cm_error_log_critical_if((array == NULL), __func__, ": array is NULL");
    cm_error_log_critical_if((compare == NULL), __func__,
                             ": search criteria is NULL");
