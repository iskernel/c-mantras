...
bool cm_yiq_is_valid(double y, double i, double q)
{
	...
	if( (double_is_within(y,PER_LOWER_LIMIT,PER_UPPER_LIMIT) == false) ||
		(double_is_within(i,YIQ_I_LOWER_LIMIT,YIQ_I_UPPER_LIMIT) == false) ||
		(double_is_within(q,YIQ_Q_LOWER_LIMIT,YIQ_Q_UPPER_LIMIT) == false) )
	{
		isValid = false;
	}
	return isValid;
...
bool cm_yuv_is_valid(double y, double u, double v)
{
	bool isValid = true;
	if( (double_is_within(y,PER_LOWER_LIMIT,PER_UPPER_LIMIT) == false) ||
		(double_is_within(u,YUV_U_LOWER_LIMIT,YUV_U_UPPER_LIMIT) == false) ||
		(double_is_within(v,YUV_V_LOWER_LIMIT,YUV_V_UPPER_LIMIT) == false) )
	{
		isValid = false;
	}
	return isValid;
