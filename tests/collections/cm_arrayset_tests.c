...
static void cm_arrayset_resize__normal_scenario__is_created(void)
{
	...
	assert_is_true( ( (set != NULL) && (set->capacity == 6) && (*(int*)set->content[1] == 1)
					   && (*(int*)set->content[5] == 5)), __func__);
...
static void cm_arrayset_remove__existing_element__is_removed(void)
{
	...
	assert_is_true( (condition && (set->size == 4) && (*(int*)set->content[1] == 2) ), __func__);
...
static void cm_arrayset_copy__normal_scenario__is_copied(void)
{
	...
	condition = (other_set != NULL) && (other_set->capacity == 10) && (other_set->size == 5)
